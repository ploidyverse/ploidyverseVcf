...
Note: I would like to include some custom functions that construct the 
matrix-lists that will go into the `geno` slots.  From polyRAD it would be a
3D array of genotype posterior probabilties that need to be formatted to go
into the GP slot, and a matrix of posterior mean genotypes to go into the GN
slot.  Some feedback from other package developers might be helpful here to 
...
If you want ao `TRUE` or `FALSE` value to tell you whether the `VCF` object
